(()=>{
  ...
  style.textContent=`
    ...
    .seen-us-viewport{position:relative;width:100%;overflow:hidden}
    .seen-us-viewport::before,.seen-us-viewport::after{content:'';position:absolute;z-index:6;top:-38%;bottom:-38%;width:clamp(5.5rem,9vw,10rem);pointer-events:none;backdrop-filter:blur(34px);-webkit-backdrop-filter:blur(34px)}
    .seen-us-viewport::before{left:-1.1rem;background:linear-gradient(90deg,rgba(9,9,9,.98) 0%,rgba(9,9,9,.76) 20%,rgba(9,9,9,.34) 57%,rgba(9,9,9,.08) 82%,rgba(9,9,9,0) 100%);mask-image:linear-gradient(90deg,#000 0%,#000 18%,rgba(0,0,0,.86) 46%,rgba(0,0,0,.42) 76%,transparent 100%)}
    .seen-us-viewport::after{right:-1.1rem;background:linear-gradient(270deg,rgba(9,9,9,.98) 0%,rgba(9,9,9,.76) 20%,rgba(9,9,9,.34) 57%,rgba(9,9,9,.08) 82%,rgba(9,9,9,0) 100%);mask-image:linear-gradient(270deg,#000 0%,#000 18%,rgba(0,0,0,.86) 46%,rgba(0,0,0,.42) 76%,transparent 100%)}
    ...
    .seen-us-item{--seen-scale:.9;flex:0 0 clamp(10rem,13.5vw,14rem);margin:0;text-align:center;transform:scale(var(--seen-scale));transform-origin:center center;opacity:1;transition:transform .16s linear;will-change:transform}
    ...
    @media(max-width:1050px){
      .seen-us-inner{padding-inline:clamp(1.75rem,4vw,2.5rem)}
    }
    @media(max-width:760px){
      .seen-us{padding:5rem 0 6rem}
      .seen-us-inner{padding-inline:.75rem}
      .seen-us-kicker{margin-bottom:2.1rem;font-size:.68rem}
      .seen-us-track{gap:1.15rem;padding-block:1.5rem 2rem;animation-duration:34s}
      .seen-us-item{flex-basis:9.2rem}
      .seen-us-item img{height:5.8rem}
      .seen-us-viewport::before,.seen-us-viewport::after{width:4.5rem;top:-28%;bottom:-28%;backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px)}
    }
    @media(prefers-reduced-motion:reduce){
      .seen-us-viewport{overflow-x:auto}
      .seen-us-viewport::before,.seen-us-viewport::after{display:none}
      .seen-us-track{animation:none}
      .seen-us-item{--seen-scale:1}
    }
  `;
  ...
  function updateDepth(){
    ...
    const radius=Math.max(1,box.width*.58);

    items.forEach(item=>{
      const r=item.getBoundingClientRect();
      const itemCenter=r.left+r.width/2;
      const distance=Math.min(1,Math.abs(itemCenter-center)/radius);
      const focus=Math.pow(1-distance,1.25);
      const scale=.86+focus*.16;
