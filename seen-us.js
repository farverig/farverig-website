(()=>{
  ...
  [...logos,...logos].forEach((file,index)=>{const item=makeItem(file);if(index>=logos.length)item.setAttribute('aria-hidden','true');track.appendChild(item);});viewport.appendChild(track);inner.append(heading,viewport);section.appendChild(inner);offers.after(section);
  const style=document.createElement('style');style.textContent=`
    .seen-us{position:relative;z-index:2;background:transparent;overflow:hidden;padding:clamp(2rem,3.5vh,3rem) 0 clamp(7rem,10vh,9rem)}
    .seen-us-inner{position:relative;z-index:8;padding-inline:clamp(5rem,8vw,9rem)}
    .seen-us-kicker{margin:0 0 clamp(1.1rem,1.8vh,1.55rem);color:rgba(255,255,255,.72);font:400 clamp(.72rem,.82vw,.84rem)/1.4 monospace;letter-spacing:.08em;text-transform:uppercase}
    ...
    .seen-us-track{display:flex;width:max-content;align-items:center;gap:clamp(1.25rem,1.8vw,2rem);padding:clamp(.8rem,1.5vh,1.2rem) 0 clamp(1.5rem,2.5vh,2rem);will-change:transform}
    .seen-us-item{--seen-scale:.72;--seen-z:0px;flex:0 0 clamp(13.5rem,18vw,19rem);margin:0;text-align:center;transform:translateZ(var(--seen-z)) scale(var(--seen-scale));transform-origin:center;opacity:1;transition:transform .12s linear,filter .18s linear;will-change:transform,filter}
    .seen-us-item img{display:block;width:100%;height:clamp(8rem,11vw,12rem);object-fit:contain;object-position:center;opacity:1;filter:grayscale(1) saturate(0) brightness(1.65) contrast(.92);transition:filter .28s ease,transform .28s ease}
    ...
    @media(max-width:760px){.seen-us{padding:2.4rem 0 5.5rem}.seen-us-inner{padding-inline:1.15rem}.seen-us-kicker{margin-bottom:1.2rem;font-size:.68rem}.seen-us-track{gap:1rem;padding-block:.8rem 1.4rem}.seen-us-item{flex-basis:11.5rem}.seen-us-item img{height:7.2rem}.seen-us-viewport{-webkit-mask-image:linear-gradient(90deg,transparent 0%,rgba(0,0,0,.5) 5%,#000 13%,#000 87%,rgba(0,0,0,.5) 95%,transparent 100%);mask-image:linear-gradient(90deg,transparent 0%,rgba(0,0,0,.5) 5%,#000 13%,#000 87%,rgba(0,0,0,.5) 95%,transparent 100%)}}
